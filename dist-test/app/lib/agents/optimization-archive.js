...
exports.OptimizationArchiveAgent = void 0;
const base_1 = require("./base");
const types_1 = require("../api/types");
class OptimizationArchiveAgent extends base_1.BaseAgent {
    constructor() {
        super(types_1.AgentType.OPTIMIZATION_ARCHIVE);
    }
    formatInput(context) {
        const contentCreation = context.previousOutputs.get(types_1.AgentType.CONTENT_CREATION);
        const copyGeneration = context.previousOutputs.get(types_1.AgentType.COPY_GENERATION);
...
出力は以下のJSON形式でお願いします：
{
  ...
  "automation_status": {
    ...
    validateOutput(output) {
        ...
        if (!output.optimization_results || typeof output.optimization_results !== 'object')
            return false;
        // performance_metricsの検証
        const metrics = output.optimization_results.performance_metrics;
        if (!metrics || typeof metrics !== 'object')
            return false;
        if (!metrics.content_performance || typeof metrics.content_performance !== 'object')
            return false;
        if (!metrics.overall_roi || typeof metrics.overall_roi !== 'object')
            return false;
        // success_patternsの検証
        if (!Array.isArray(output.optimization_results.success_patterns))
            return false;
        // improvement_recommendationsの検証
        if (!Array.isArray(output.optimization_results.improvement_recommendations))
            return false;
        // template_archiveの検証
        if (!output.template_archive || typeof output.template_archive !== 'object')
            return false;
        if (!Array.isArray(output.template_archive.master_templates))
            return false;
        if (!Array.isArray(output.template_archive.version_history))
            return false;
        // automation_statusの検証
        if (!output.automation_status || typeof output.automation_status !== 'object')
            return false;
        if (!Array.isArray(output.automation_status.automated_processes))
            return false;
        if (!Array.isArray(output.automation_status.manual_review_points))
            return false;
        if (!output.automation_status.next_optimization_cycle)
            return false;
        return true;
    }
}
exports.OptimizationArchiveAgent = OptimizationArchiveAgent;
