...
class ContentCreationAgent extends base_1.BaseAgent {
    constructor() {
        super(types_1.AgentType.CONTENT_CREATION);
    }
    formatInput(context) {
        const businessStrategy = context.previousOutputs.get(types_1.AgentType.BUSINESS_STRATEGY);
        const templateOptimization = context.previousOutputs.get(types_1.AgentType.TEMPLATE_OPTIMIZATION);
  ...
  "content_calendar": {
    ...
    validateOutput(output) {
        if (!output || typeof output !== 'object')
            return false;
        // landing_pageの検証
        if (!output.landing_page || typeof output.landing_page !== 'object')
            return false;
        const lp = output.landing_page;
        if (!lp.headline || typeof lp.headline !== 'string')
            return false;
        if (!lp.subheadline || typeof lp.subheadline !== 'string')
            return false;
        if (!Array.isArray(lp.sections))
            return false;
        if (!lp.design_notes || typeof lp.design_notes !== 'string')
            return false;
        // sns_contentの検証
        if (!output.sns_content || typeof output.sns_content !== 'object')
            return false;
        if (!Array.isArray(output.sns_content.twitter))
            return false;
        if (!Array.isArray(output.sns_content.instagram))
            return false;
        // email_sequenceの検証
        if (!Array.isArray(output.email_sequence))
            return false;
        if (output.email_sequence.length < 7)
            return false;
        for (const email of output.email_sequence) {
            if (typeof email.email_number !== 'number')
                return false;
            if (!email.send_timing || typeof email.send_timing !== 'string')
                return false;
            if (!email.subject || typeof email.subject !== 'string')
                return false;
            if (!email.preheader || typeof email.preheader !== 'string')
                return false;
            if (!email.content || typeof email.content !== 'string')
                return false;
            if (!email.cta || typeof email.cta !== 'string')
                return false;
            if (!email.purpose || typeof email.purpose !== 'string')
                return false;
        }
        // content_calendarの検証
        if (!output.content_calendar || typeof output.content_calendar !== 'object')
            return false;
        return true;
    }
}
