...
class BusinessStrategyAgent extends base_1.BaseAgent {
    constructor() {
        super(types_1.AgentType.BUSINESS_STRATEGY);
    }
    formatInput(context) {
  ...
  "roi_projection": {
    ...
    validateOutput(output) {
        if (!output || typeof output !== 'object')
            return false;
        // product_lineupの検証
        if (!output.product_lineup || typeof output.product_lineup !== 'object')
            return false;
        ...
        for (const product of products) {
            ...
            if (typeof p.price !== 'number')
                return false;
            if (!p.purpose || typeof p.purpose !== 'string')
                return false;
            if (!p.content || typeof p.content !== 'string')
                return false;
            if (typeof p.profit_margin !== 'number')
                return false;
        }
        // sales_funnelの検証
        if (!output.sales_funnel || typeof output.sales_funnel !== 'object')
            return false;
        ...
        for (const stage of stages) {
            ...
            if (!Array.isArray(s.channels))
                return false;
            if (!Array.isArray(s.content_types))
                return false;
            if (!s.kpi || typeof s.kpi !== 'string')
                return false;
        }
        // roi_projectionの検証
        if (!output.roi_projection || typeof output.roi_projection !== 'object')
            return false;
        const roi = output.roi_projection;
        if (typeof roi.monthly_target_leads !== 'number')
            return false;
        if (!roi.conversion_rates || typeof roi.conversion_rates !== 'object')
            return false;
        if (typeof roi.monthly_revenue !== 'number')
            return false;
        if (!roi.cost_structure || typeof roi.cost_structure !== 'object')
            return false;
        if (typeof roi.net_profit !== 'number')
            return false;
        if (typeof roi.roi_percentage !== 'number')
            return false;
        return true;
    }
}
