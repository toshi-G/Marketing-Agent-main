...
class MarketResearchAgent extends base_1.BaseAgent {
    constructor() {
        super(types_1.AgentType.MARKET_RESEARCH);
    }
    formatInput(context) {
  ...
  "analysis_summary": "市場分析の総括"
    ...
    validateOutput(output) {
        if (!output || typeof output !== 'object')
            return false;
        // 必須フィールドのチェック
        if (!Array.isArray(output.recommended_genres))
            return false;
        if (!output.analysis_summary || typeof output.analysis_summary !== 'string')
            return false;
        // 各ジャンルの検証
        for (const genre of output.recommended_genres) {
            if (!genre.genre || typeof genre.genre !== 'string')
                return false;
            if (typeof genre.trend_score !== 'number')
                return false;
            if (typeof genre.profitability_score !== 'number')
                return false;
            if (!genre.competition_level || typeof genre.competition_level !== 'string')
                return false;
            if (!genre.market_size || typeof genre.market_size !== 'string')
                return false;
            if (!genre.target_audience || typeof genre.target_audience !== 'string')
                return false;
            if (!genre.reason || typeof genre.reason !== 'string')
                return false;
            if (!Array.isArray(genre.keywords))
                return false;
        }
        return true;
    }
