...
exports.TemplateOptimizationAgent = void 0;
const base_1 = require("./base");
const types_1 = require("../api/types");
class TemplateOptimizationAgent extends base_1.BaseAgent {
    constructor() {
        super(types_1.AgentType.TEMPLATE_OPTIMIZATION);
    }
    formatInput(context) {
        const nlpClassification = context.previousOutputs.get(types_1.AgentType.NLP_CLASSIFICATION);
...
出力は以下のJSON形式でお願いします：
{
  ...
  "testing_recommendations": ["A/Bテスト案1", "A/Bテスト案2"]
    ...
    validateOutput(output) {
        ...
        if (!Array.isArray(output.optimized_templates))
            return false;
        if (output.optimized_templates.length !== 5)
            return false;
        for (const template of output.optimized_templates) {
            ...
            if (typeof template.success_rate !== 'number')
                return false;
            if (!Array.isArray(template.best_for))
                return false;
            if (!template.structure || typeof template.structure !== 'object')
                return false;
            if (!Array.isArray(template.variables))
                return false;
            if (!template.example_usage || typeof template.example_usage !== 'string')
                return false;
            if (!template.metrics || typeof template.metrics !== 'object')
                return false;
        }
        // selection_criteriaの検証
        if (!output.selection_criteria || typeof output.selection_criteria !== 'object')
            return false;
        if (typeof output.selection_criteria.performance_threshold !== 'number')
            return false;
        if (typeof output.selection_criteria.versatility_score !== 'number')
            return false;
        if (typeof output.selection_criteria.ease_of_use !== 'number')
            return false;
        // testing_recommendationsの検証
        if (!Array.isArray(output.testing_recommendations))
            return false;
        return true;
    }
}
exports.TemplateOptimizationAgent = TemplateOptimizationAgent;
