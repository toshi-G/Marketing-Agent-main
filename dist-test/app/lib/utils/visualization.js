...
exports.transformMarketResearchData = transformMarketResearchData;
exports.transformContentAnalysisData = transformContentAnalysisData;
exports.transformTemplateOptimizationData = transformTemplateOptimizationData;
exports.transformBusinessStrategyData = transformBusinessStrategyData;
exports.transformContentCreationData = transformContentCreationData;
exports.transformCopyGenerationData = transformCopyGenerationData;
exports.transformAgentDataToVisualization = transformAgentDataToVisualization;
exports.createDashboardData = createDashboardData;
exports.exportDataAsJSON = exportDataAsJSON;
exports.exportDataAsCSV = exportDataAsCSV;
const types_1 = require("../api/types");
// 市場調査データの変換
function transformMarketResearchData(data) {
    return {
...
// コンテンツ分析データの変換
function transformContentAnalysisData(scrapingData, nlpData) {
    // プラットフォーム別統計
...
// テンプレート最適化データの変換
function transformTemplateOptimizationData(data) {
...
// ビジネス戦略データの変換
function transformBusinessStrategyData(data) {
    const productLineup = [
        {
...
// コンテンツ作成データの変換
function transformContentCreationData(data) {
    const contentTypes = [
        {
            type: 'Landing Page',
            count: 1,
...
// コピー生成データの変換
function transformCopyGenerationData(data) {
...
// エージェントデータの統合変換
function transformAgentDataToVisualization(agents) {
    const visualization = {};
        ...
        try {
            ...
            switch (agent.type) {
                ...
                case types_1.AgentType.NLP_CLASSIFICATION:
                    ...
                    const scrapingAgent = agents.find(a => a.type === types_1.AgentType.CONTENT_SCRAPING);
...
// ダッシュボードデータの作成
function createDashboardData(workflowId, workflowName, status, agents, completedAt) {
    const visualization = transformAgentDataToVisualization(agents);
    const completedAgents = agents.filter(agent => agent.status === types_1.WorkflowStatus.COMPLETED).length;
    ...
    const endTimes = agents.map(agent => agent.completedAt ? new Date(agent.completedAt).getTime() : null).filter((time) => time !== null);
    ...
    const exportData = {
        templates: agents
            .filter(agent => agent.type === types_1.AgentType.TEMPLATE_OPTIMIZATION && agent.output)
            .map(agent => {
            try {
            ...
            .filter(Boolean),
        content: agents
            .filter(agent => agent.type === types_1.AgentType.CONTENT_CREATION && agent.output)
            .map(agent => {
            try {
                return typeof agent.output === 'string' ? JSON.parse(agent.output) : agent.output;
            }
