...
export class CopyGenerationAgent extends BaseAgent {
  constructor() {
    super(AgentType.COPY_GENERATION);
  }
...
出力は以下のJSON形式でお願いします：
{
  ...
  parseOutput(response: string): CopyGenerationOutput {
  ...
  validateOutput(output: any): boolean {
    ...
    for (const category of hookCategories) {
      if (!Array.isArray(output[category])) return false;
      if (output[category].length < 20) return false;
      
      for (const hook of output[category]) {
        if (!hook.title || typeof hook.title !== 'string') return false;
        if (!hook.hook_text || typeof hook.hook_text !== 'string') return false;
        if (!hook.emotion_trigger || typeof hook.emotion_trigger !== 'string') return false;
        if (!hook.target_audience || typeof hook.target_audience !== 'string') return false;
        if (typeof hook.expected_ctr !== 'number') return false;
      }
    }
    
    // performance_predictionの検証
    if (!output.performance_prediction || typeof output.performance_prediction !== 'object') return false;
    const perf = output.performance_prediction;
    if (!perf.best_performing_category || typeof perf.best_performing_category !== 'string') return false;
    if (typeof perf.overall_avg_ctr !== 'number') return false;
    if (!Array.isArray(perf.a_b_test_recommendations)) return false;
    
    // usage_guidelinesの検証
    if (!output.usage_guidelines || typeof output.usage_guidelines !== 'object') return false;
