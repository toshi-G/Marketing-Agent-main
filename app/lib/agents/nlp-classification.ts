...
import { AgentType, NLPClassificationOutput, ContentScrapingOutput } from '../api/types';

export class NLPClassificationAgent extends BaseAgent {
  constructor() {
    super(AgentType.NLP_CLASSIFICATION);
  }
  
  formatInput(context: AgentContext): string {
    const contentScraping = context.previousOutputs.get(AgentType.CONTENT_SCRAPING) as ContentScrapingOutput;
...
出力は以下のJSON形式でお願いします：
{
  ...
  parseOutput(response: string): NLPClassificationOutput {
  ...
  validateOutput(output: any): boolean {
    ...
    // classified_dataの検証
    if (!output.classified_data || typeof output.classified_data !== 'object') return false;
    
    // by_appeal_typeの検証
    const appealTypes = ['benefit', 'emotional', 'authority', 'scarcity', 'social_proof'];
    for (const type of appealTypes) {
      if (!Array.isArray(output.classified_data.by_appeal_type[type])) return false;
    }
    
    // by_emotionの検証
    const emotionTypes = ['positive', 'negative', 'neutral'];
    for (const type of emotionTypes) {
      if (!Array.isArray(output.classified_data.by_emotion[type])) return false;
    }
    
    // by_structureの検証
    const structureTypes = ['problem_focused', 'solution_focused', 'story_based', 'list_based', 'comparison_based'];
    for (const type of structureTypes) {
      if (!Array.isArray(output.classified_data.by_structure[type])) return false;
    }
    
    // classification_statsの検証
    if (!output.classification_stats || typeof output.classification_stats !== 'object') return false;
    if (typeof output.classification_stats.total_classified !== 'number') return false;
    if (typeof output.classification_stats.accuracy_rate !== 'number') return false;
    if (!Array.isArray(output.classification_stats.top_patterns)) return false;
